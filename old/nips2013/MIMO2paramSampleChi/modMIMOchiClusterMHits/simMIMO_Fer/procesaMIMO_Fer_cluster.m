function procesaMIMO_Fer_cluster(itH,SNR)
...
Nfin_ = zeros(1,1,10);%numero de transmisores estimado
Mfin_ = zeros(1,1,10);%binario
MSE_ = zeros(1,1,10);
BER_ = zeros(1,1,10);
pOnOff_ = zeros(1,1,10);
mem_ = zeros(1,1,10,3);
flag=0;
BASEDIR=['/export/clusterdata/ivalera/modMIMOFerN3T400_L10/SNR' num2str(SNR) '_itH'  num2str(itH)];
if ( isdir([BASEDIR '/../Resultados']) == 0 )
    mkdir([BASEDIR '/../Resultados']);
end

if 1%exist([BASEDIR '/../Resultados/SNR' num2str(SNR) '_itH'  num2str(itH) '.mat'],'file')==0
    for itcluster=1:10
    ...
    save ([BASEDIR '/../Resultados/SNR' num2str(SNR) '_itH'  num2str(itH) '.mat'], 'BER_','MSE_','Mfin_','Nfin_','pOnOff_','mem_')
end
