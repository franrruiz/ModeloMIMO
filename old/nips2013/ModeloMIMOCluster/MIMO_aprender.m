function  [Zest,Hest,MestIt,pX_ZIt,rest,nest] = MIMO_aprender(X,s2x,s2h,alpha,gamma1,gamma2,beta,pii,lambda,Nsim, sim,BASEDIR)
...
for it=1:Nsim
    %% pasos a) y b): Gibbs
   [Zest, rest,MestIt(it),pX_ZIt(it),nest]= MIMO_GibbsZ(X,s2x,s2h,alpha,gamma1,gamma2,beta,pii,lambda,1,Zest,nest,rest);
    %% Muestreo r
    for m=1:MestIt(it)
        rnew=rest;
    ...
    save([BASEDIR '/Sim' num2str(sim) 'It' num2str(it)], 'rest', 'Zest')
