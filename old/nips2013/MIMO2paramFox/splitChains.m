function [Snew nnew rnew pSplit]=splitChains(S,X,n,m,r,chi,tau,nu, alpha1,gamma1,gamma2, t1,t2)
[M,T]=size(S);
Snew=[S; zeros(1,T)];
Snew(m,:)=0;
    ...
    if S(m,t)==0
        ...
        if t==t1 
            if sum(Snew(m1,1:t-1))==0            
                ...
                if Snew(m1,t-1)==0
                    p=(log(sum(nnew(1,2:end,m1)))-log(sum(nnew(1,:,m1))+alpha1)-log(2))*ones(1,3);

                else
                    p=(log(sum(nnew(Snew(m1,t-1)+1,2:3,m1))+gamma2)-log(sum(nnew(Snew(m1,t-1)+1,:,m1))+gamma1+gamma2)-log(2))*ones(1,3); 
            ...
            if sum(Snew(m2,1:t-1))==0            
                ...
                if Snew(m2,t-1)==0
                    p=p+[log(nnew(1,1,m2)+alpha1)-log(sum(nnew(1,:,m2))+alpha1) (log(sum(nnew(1,2:end,m2)))-log(sum(nnew(1,:,m2))+alpha1)-log(2))*ones(1,2)];

                else
                    p=p+[ log(nnew(Snew(m2,t-1)+1,1,m2)+gamma1)-log(sum(nnew(Snew(m2,t-1)+1,:,m2))+gamma1+gamma2) (log(sum(nnew(Snew(m2,t-1)+1,2:3,m2))+gamma2)-log(sum(nnew(Snew(m2,t-1)+1,:,m2))+gamma1+gamma2)-log(2))*ones(1,2)]; 
        ...
        elseif t==t2
            if sum(Snew(m2,1:t-1))==0            
                ...
                if Snew(m2,t-1)==0
                    p=(log(sum(nnew(1,2:end,m2)))-log(sum(nnew(1,:,m2))+alpha1)-log(2))*ones(1,3);

                else
                    p=(log(sum(nnew(Snew(m2,t-1)+1,2:3,m2))+gamma2)-log(sum(nnew(Snew(m2,t-1)+1,:,m2))+gamma1+gamma2)-log(2))*ones(1,3); 
            ...
            if sum(Snew(m1,1:t-1))==0            
                ...
                if Snew(m1,t-1)==0
                    p=p+[log(nnew(1,1,m1)+alpha1)-log(sum(nnew(1,:,m1))+alpha1) (log(sum(nnew(1,2:end,m1)))-log(sum(nnew(1,:,m1))+alpha1)-log(2))*ones(1,2)];

                else
                    p=p+[ log(nnew(Snew(m1,t-1)+1,1,m1)+gamma1)-log(sum(nnew(Snew(m1,t-1)+1,:,m1))+gamma1+gamma2) (log(sum(nnew(Snew(m1,t-1)+1,2:3,m1))+gamma2)-log(sum(nnew(Snew(m1,t-1)+1,:,m1))+gamma1+gamma2)-log(2))*ones(1,2)]; 
