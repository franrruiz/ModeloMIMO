function [Zest, rest,Mest,pX_Z,nest]= MIMO_GibbsZ(X,s2x,s2h,alpha1, alpha2,gamma1,gamma2,beta,pii,lambda,Nsim,Zest,nest,rest)
...
%                     p=p/sum(p);
        ...
        pA= mLik_TB(X,Znew,rnew,s2x,s2h)-mLik_TB(X,Zest,rest,s2x,s2h)+Mnew*log(alpha1*alpha2)-log(factorial(sum(snew==1)))-log(factorial(sum(snew==2)))+Mnew*log(gamma1)-Mnew*log(T-1)-Mnew*log(gamma1+gamma2)-log((1-pii)*poisspdf(Mnew,alpha2*lambda/(T-1))+pii*(Mnew==1));
        acepto=rand<exp(pA);
        if acepto
            Zest=Znew;
            rest=rnew;
