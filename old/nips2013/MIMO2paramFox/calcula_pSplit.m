function pSplit=calcula_pSplit(S,Snew,X,m1,m2,r,chi,tau,nu,alpha1,gamma1,gamma2, t1,t2, flagSigno)
[M,T]=size(Snew);
...
if t==t1 
    ...
    s1=Snew(end,t)*ones(1,3);
    s2=[ 0 Snew(end,t) 3-Snew(end,t)];
    for i=1:3
        Saux=S;
        Saux(m1,t)=s1(i);
        Saux(m2,t)=s2(i);
...
elseif t==t2 
    ...
    s2=Snew(end,t)*ones(1,3);
    s1=[ 0 Snew(end,t) 3-Snew(end,t)];
    for i=1:3
        Saux=S;
        Saux(m1,t)=s1(i);
        Saux(m2,t)=s2(i);
...
for t=2:T
    if t==t1 
        if sum(S(m1,1:t-1))==0            
            ...
            if S(m1,t-1)==0
                p=(log(sum(n(1,2:end,1)))-log(sum(n(1,:,1))+alpha1)-log(2))*ones(1,3);

            else
                p=(log(sum(n(S(m1,t-1)+1,2:3,1))+gamma2)-log(sum(n(S(m1,t-1)+1,:,1))+gamma1+gamma2)-log(2))*ones(1,3); 
        ...
        if sum(S(m2,1:t-1))==0            
            ...
            if S(m2,t-1)==0
                p=p+[log(n(1,1,2)+alpha1)-log(sum(n(1,:,2))+alpha1) (log(sum(n(1,2:end,2)))-log(sum(n(1,:,2))+alpha1)-log(2))*ones(1,2)];

            else
                p=p+[ log(n(S(m2,t-1)+1,1,2)+gamma1)-log(sum(n(S(m2,t-1)+1,:,2))+gamma1+gamma2) (log(sum(n(S(m2,t-1)+1,2:3,2))+gamma2)-log(sum(n(S(m2,t-1)+1,:,2))+gamma1+gamma2)-log(2))*ones(1,2)]; 
    ...
    elseif t==t2
        if sum(S(m2,1:t-1))==0            
            ...
            if S(m2,t-1)==0
                p=(log(sum(n(1,2:end,2)))-log(sum(n(1,:,2))+alpha1)-log(2))*ones(1,3);

            else
                p=(log(sum(n(S(m2,t-1)+1,2:3,2))+gamma2)-log(sum(n(S(m2,t-1)+1,:,2))+gamma1+gamma2)-log(2))*ones(1,3); 
        ...
        if sum(S(m1,1:t-1))==0            
            ...
            if S(m1,t-1)==0
                p=p+[log(n(1,1,1)+alpha1)-log(sum(n(1,:,1))+alpha1) (log(sum(n(1,2:end,1)))-log(sum(n(1,:,1))+alpha1)-log(2))*ones(1,2)];

            else
                p=p+[log(n(S(m1,t-1)+1,1,1)+gamma1)-log(sum(n(S(m1,t-1)+1,:,1))+gamma1+gamma2) (log(sum(n(S(m1,t-1)+1,2:3,1))+gamma2)-log(sum(n(S(m1,t-1)+1,:,1))+gamma1+gamma2)-log(2))*ones(1,2)]; 
