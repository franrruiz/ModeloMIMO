function [Zest, rest,Mest,pX_Z,nest]= MIMO_GibbsZ(X,s2x,s2h,alpha,gamma1,gamma2,beta,pii,lambda,Nsim,Zest,nest,rest)
        ...
        if t==1 %% Para t=1
            ...
            while m<=Mest
                ...
                if Zest(m,t+1)==0
                    ...
                    p(2:Q)= -log(2)+log(sum(nest(1,2:end,m)))+log(gamma1+nest(2:end,1,m)')-log(gamma1+gamma2+sum(nest(2:Q,:,m),2).')-log(1+sum(nest(1,:,m)));
                else
                    p(1)=  -log(2)+log(1+nest(1,1,m))-log(1+sum(nest(1,:,m)))-log(2+sum(nest(1,:,m)));
                    p(2:Q)=  -log(4)+log(gamma2+sum(nest(2:end,2:end,m),2)')-log(gamma1+gamma2+sum(nest(2:Q,:,m),2).')-log(1+sum(nest(1,:,m)));
                end
...
%                 p=p/sum(p);
                ...
                for k=1:Q
                    Zest(m,t)=k-1;
                    fk(k)= mLik_TB(X,Zest,rest,s2x,s2h);
                end
                prob = fk+ p;
                maximo = max(prob);
        ...
        elseif t==T %% t=T
            ...
            while m<=Mest
                ...
                if(j==0)
                    ...
                    p(2:Q)= -log(2)+log(sum(nest(1,2:end,m)))-log(1+sum(nest(1,:,m)));
%                     maximo = max(p);
%                     p=exp(p-maximo);
%                     p=p/sum(p);
                else
...
%                     p=p/sum(p);
            ...
            while m<=Mest
                ...
                if(j==0)
                    ...
                    if Zest(m,t+1)==0
                        p(1)= log(gamma1+nest(j+1,1,m))- log(gamma1+gamma2+sum(nest(j+1,:,m),2))-log(1+sum(nest(1,:,m)))+log(1+nest(1,1,m));
                        ...
                        p(1)= -log(2)+ log(gamma1+nest(j+1,1,m))- log(gamma1+gamma2+sum(nest(j+1,:,m),2))+log (sum(nest(1,2:Q,m)))-log(1+sum(nest(1,:,m)));
        ...
        %% A?adimos cadenas
        aux=(rand<pii);
        Mnew=(1-aux)*poissrnd(alpha*lambda/(T-1))+aux;
        rnew=[rest poissrnd(beta,1,Mnew)+1];
        snew=round(rand(Mnew,1))+1;
        Znew= [Zest; zeros(Mnew,T)];
        Znew(Mest+1:Mest+Mnew,t)=snew;
        
        pA= mLik_TB(X,Znew,rnew,s2x,s2h)-mLik_TB(X,Zest,rest,s2x,s2h)+Mnew*log(alpha)-log(factorial(sum(snew==1)))-log(factorial(sum(snew==2)))+Mnew*log(gamma1)-Mnew*log(T-1)-Mnew*log(gamma1+gamma2)-log((1-pii)*poisspdf(Mnew,alpha*lambda/(T-1))+pii*(Mnew==1));
        acepto=rand<exp(pA);
        if acepto
            Zest=Znew;
            rest=rnew;
...
pX_Z= mLik_TB(X,Zest,rest,s2x,s2h);
%Mest
