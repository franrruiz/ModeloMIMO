function simClusterWise_yeti(T,Nt,Nr,M,L,Niter,lHead,onOffModel,Nparticles,blockNtSize,flagParallel,itCluster,simId)
% Use Niter=30000, Nparticles=3000, simId=3

addpath(genpath('/vega/dsi/users/fr2392/ModeloMIMO/matlab'));

randn('seed',round(sum(1e5*clock)+itCluster));
rand('seed',round(sum(1e5*clock)+itCluster));

saveFolder = ['/vega/dsi/users/fr2392/ModeloMIMO/results/wise/' num2str(simId) ...
...
% Load data from file
load(['/vega/dsi/users/fr2392/ModeloMIMO/data/WISEdata/preprocessed/hoh' hohChar '/T' num2str(param.T) '_Nt' num2str(param.gen.Nt) '.mat']);
