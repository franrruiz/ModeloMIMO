function simClusterSynPrueba(T,Nt,Nr,M,Ltrue,L,SNR,Niter,lHead,onOffModel,itCluster,simId)
% Try using a noise variance higher than the true one
...
param.pgas.maxM = 40;
param.pgas.flagParallel = 1;  %%% CAN BE RUN ONLY OVER MIC MACHINES
...
if(~flagRecovered)
    ...
    if(~param.infer.sampleNoiseVar)
        init.s2y = noiseVar*10^(9/10);      % INITIALIZE s2y TO THE GROUND TRUTH
    else
